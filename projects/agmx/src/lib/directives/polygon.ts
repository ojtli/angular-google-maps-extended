...
/**
 * AgmPolygon renders a polygon on a {@link AgmxMap}
 *
 * ### Example
 * ```typescript
...
export class AgmPolygon implements OnDestroy, OnChanges, AfterContentInit {
  ...
  private _pathSubscriptions: Subscription[] = [];
  ...
  private addPathEventsListeners() {
    this._pathSubscriptions.forEach((s) => s.unsubscribe());
  ...
  getPath(): Promise<Array<google.maps.LatLng>> {
    return this._polygonManager.getPath(this);
  }

  getPaths(): Promise<Array<Array<google.maps.LatLng>>> {
    return this._polygonManager.getPaths(this);
  }
}

export interface PolygonPathEvent<T extends (google.maps.LatLng | Array<google.maps.LatLng>)> {
  newArr:
    | google.maps.MVCArray<google.maps.MVCArray<google.maps.LatLng>>
    | google.maps.MVCArray<google.maps.LatLng>
    | google.maps.LatLng[][]
...
export interface PathCollectionChangePolygonPathEvent extends PolygonPathEvent <Array<google.maps.LatLng>>{
}

export interface PathChangePolygonPathEvent extends PolygonPathEvent<google.maps.LatLng> {
  pathIndex: number;
}
