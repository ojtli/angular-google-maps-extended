...
// import { google.maps.PolyMouseEvent } from '../services/google-maps-types';
...
let polylineId = 0;
/**
 * AgmPolyline renders a polyline on a {@link AgmxMap}
 *
 * ### Example
 * ```typescript
...
export class AgmPolyline implements OnDestroy, OnChanges, AfterContentInit {
  ...
  /**
   * This event is fired when a point in the path is updated
   */
  ...
  /** @internal */
  _getPoints(): google.maps.MVCArray<AgmPolylinePoint> {
    if (this.points) {
      return new google.maps.MVCArray(this.points.toArray());
    }
...
export interface PathEvent {
  newArr: google.maps.LatLng[];
  evName: 'insert_at' | 'remove_at' | 'set_at';
  index: number;
  previous?: google.maps.LatLng;
}
