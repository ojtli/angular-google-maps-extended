 ...
 * `],
 *  template: `
 *    <agmx-map [latitude]="lat" [longitude]="lng" [zoom]="zoom">
 *    </agmx-map>
 *  `
 * })
 * ```
...
export class AgmxMap implements OnChanges, OnInit, OnDestroy {
  /**
  ...
  private static _mapOptionsAttributes: string[] = [
    ...
    'mapTypeId', 'clickableIcons', 'gestureHandling', 'tilt', 'restriction', 'mapId',
  ];
   ...
   */
  @Output() mapClick: EventEmitter<google.maps.MouseEvent | google.maps.IconMouseEvent> =
  ...
  private _updateMapOptionsChanges(changes: SimpleChanges) {
    ...
    let optionKeys =
      Object.keys(changes).filter(k => AgmxMap._mapOptionsAttributes.indexOf(k) !== -1);
  ...
  private _handleMapMouseEvents() {
    ...
    regularEvents.forEach((e: Event) => {
      const s = this._mapsWrapper.subscribeToMapEvent<google.maps.MouseEvent | google.maps.IconMouseEvent>(e.name).subscribe(
